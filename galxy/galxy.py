...
class Galxy:
    @staticmethod
    async def claim(
        address: str, campaign_id: str, w: str, chain_camp: str = "MATIC"
    ) -> Dict:

        COMPLETED_DATA = (await Galxy.__validation_captcha(w))["data"]
        ...
        headers = {
            ...
            "accept": "*/*",
            "user-agent": "Mozilla/5.0 (Windows NT 6.2) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/106.0.5249.103 "
                          "Safari/537.36",
    ...
    @staticmethod
    async def get_info_by_id(id_):
        HEADERS = {
            "user-agent": "Mozilla/5.0 (Windows NT 10.0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.5112.101 "
                          "Safari/537.36",
        }
        ...
        json_data = {
            ...
            "query": "query CampaignInfo($id: ID!, $address: String!) {\n  campaign(id: $id) {\n    "
                     ...
                     "maxCount\n    usedCount\n    __typename\n  }\n  __typename\n}\n\nfragment WhitelistSubgraphFrag "
                     "on Campaign {\n  id\n  whitelistSubgraph {\n    query\n    endpoint\n    expression\n    "
                     "variable\n    __typename\n  }\n  __typename\n}\n\nfragment GamificationDetailFrag on "
                     "Gamification {\n  id\n  type\n  nfts {\n    nft {\n      id\n      animationURL\n      "
                     "category\n      powah\n      image\n      name\n      treasureBack\n      nftCore {\n        "
                     "...NftCoreInfoFrag\n        __typename\n      }\n      traits {\n        name\n        value\n  "
                     "      __typename\n      }\n      __typename\n    }\n    __typename\n  }\n  airdrop {\n    "
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                "https://graphigo.prd.galaxy.eco/query", headers=HEADERS, json=json_data
            ) as response:
                return await response.json()

    @classmethod
    async def __validation_captcha(cls, captcha_w) -> Dict:
        ...
        js_data = json.loads(resp.text.strip("geetest_{}(".format(call)).strip(")"))[
            "data"
        ]
        ...
        resp_complete = await session.get(
            "https://gcaptcha4.geetest.com/verify", params=params
        )
        VALIDATION = json.loads(
            resp_complete.text.strip("geetest_{}(".format(call)).strip(")")
        )
        return VALIDATION
    ...
    async def validation_config_w(cls, captcha_w):
        try:
            data = (await cls.__validation_captcha(captcha_w))["status"]
            if data == "success":
            ...
            logger.error(
                "VALIDATION CAPTCHA | FALSE | UNKNOWN PARAMETERS | {}".format(data)
            )
            return False
